...
test('should add a unit', () => {
  convert.addUnit('lyes', 0.450496);
  const units = convert.units();
  expect(units[units.length - 1]).toBe('lyes');
});


test('should remove an unit', () => {
  convert.removeUnit('lyes');
  expect(convert.units().includes('lyes')).not.toBeTruthy();
});
